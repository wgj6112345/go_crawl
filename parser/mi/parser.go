...
const (
	urlPrefix = "https://list.mi.com/"
	reLevel1  = `<li>[\d\D]*?<a href="([^"]+)"[\d\D]*?><img[\d\D]*?></a>[\d\D]*?<a[\d\D]*?>[\d\D]*?</a>[\d\D]*?</li>`
	reLevel2  = ``
)
...
func ParseLevel3(body []byte) (result model.ParseResult) {
	// 预编译
...
func ParseLevel4(body []byte) (result model.ParseResult) {
	return
}
